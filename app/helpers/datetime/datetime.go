...
// ParseLocal - converts time.Time UTC to local time
func ParseLocal(inputTime time.Time) time.Time {
	_, offset := time.Now().Zone() // zone, offset

	return inputTime.Add(time.Second * time.Duration(offset))
}

// ParseLocalString - converts string to local time
func ParseLocalString(timeStr string) time.Time {
...
func ToScyllaTimestamp(timeStr string) time.Time {
	t := ParseLocalString(fmt.Sprintf("%s 23:59:59", timeStr))
	//t, _ := time.Parse("2006-01-02 15:04:05", timeStr) // NOTE: reference by me, @author: karl
